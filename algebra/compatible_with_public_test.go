...
func TestCompatibleWithAliasing(t *testing.T) {
	shared := &omnist.Record{
		Name: "Shared",
		Fields: []omnist.Field{
			{Label: "value", Type: omnist.ScalarType(omnist.KindString, false), Cardinality: omnist.DefaultCardinality()},
		},
	}
	root := &omnist.Record{
		Name: "Root",
		Fields: []omnist.Field{
			...
			{Label: "one", Type: omnist.RefType("Alias1"), Cardinality: omnist.DefaultCardinality()},
			{Label: "two", Type: omnist.RefType("Alias2"), Cardinality: omnist.DefaultCardinality()},
		},
	}
	a := omnist.Schema{
		Root: "Root",
